  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#050505', fontFamily: 'Inter, sans-serif' }}>
        <div style={{ background: 'rgba(255,255,255,0.04)', borderBottom: '1px solid #e5e7eb', padding: '14px 24px', display: 'flex', alignItems: 'center', gap: 16 }}>
          <Link href="/dashboard" style={{ display: 'flex', alignItems: 'center', gap: 6, textDecoration: 'none', color: 'rgba(237,237,237,0.4)', fontSize: 13 }}><ArrowLeft size={14} /> Dashboard</Link>
          <h1 style={{ fontSize: 16, fontWeight: 800, color: '#ededed' }}>Analytics</h1>
        </div>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 24px' }}>
          {loading ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 28 }}>
              {[...Array(4)].map((_,i) => <div key={i} style={{ height: 110, background: 'rgba(255,255,255,0.04)', borderRadius: 14, border: '1px solid rgba(255,255,255,0.07)', animation: 'pulse 1.5s ease infinite' }} />)}
            </div>
          ) : (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px,1fr))', gap: 14, marginBottom: 28 }}>
                {[
                  ...
                  <div key={i} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 22 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                      <span style={{ fontSize: 12, color: 'rgba(237,237,237,0.4)', fontWeight: 500 }}>{label}</span>
                    ...
                    <div style={{ fontSize: 30, fontWeight: 900, color: '#ededed', letterSpacing: '-0.02em' }}>{typeof value === 'number' ? value.toLocaleString() : value}</div>
                  </div>
                ))}
              </div>
              ...
              {topPages.length > 0 && (
                <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, overflow: 'hidden' }}>
                  <div style={{ padding: '18px 20px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 14, fontWeight: 700, color: '#ededed' }}>Top Performing Pages</span>
                  </div>
                  {topPages.map((p, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: '1px solid #f9fafb' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
                        <span style={{ fontSize: 12, color: 'rgba(237,237,237,0.3)', fontWeight: 700, width: 20 }}>#{i+1}</span>
                        <div style={{ minWidth: 0 }}>
                          <div style={{ fontSize: 13, fontWeight: 600, color: '#ededed', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 400 }}>{p.keyword || p.title || 'Untitled'}</div>
                          <div style={{ fontSize: 11, color: 'rgba(237,237,237,0.3)', marginTop: 2 }}>{p.domain}</div>
                        </div>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexShrink: 0 }}>
                        <span style={{ fontSize: 13, fontWeight: 700, color: '#4f46e5' }}>{p.views?.toLocaleString() || 0} views</span>
                        {p.url && <a href={p.url} target="_blank" rel="noreferrer" style={{ color: 'rgba(237,237,237,0.3)' }}><ArrowUpRight size={13} /></a>}
                      </div>
                    </div>
                  ))}
                </div>
